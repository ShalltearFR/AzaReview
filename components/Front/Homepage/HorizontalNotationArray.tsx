interface HorizontalNotationArrayProps {
  translate: Array<string>;
}

const HorizontalNotationArray: React.FC<HorizontalNotationArrayProps> = ({
  translate,
}) => {
  return (
    <table className="hidden mmd:table w-[750px] text-center rounded-2xl overflow-hidden">
      <thead>
        <tr className="[&_th]:border-b [&_th]:border-white">
          ...
          <th className="w-1/5 bg-blue">{translate[0] ?? ""}</th>
          <th className="w-1/5 bg-black">&gt;A+</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          ...
          <td className="w-auto bg-black">{translate[2] ?? ""}</td>
          <td className="w-auto bg-blue">Correct</td>
          <td className="w-auto bg-black">{translate[1] ?? ""}</td>
        </tr>
      </tbody>
    </table>
  );
